...
The consumer-facing handbook is shipped alongside this hook in
docs/handbook.md.

Usage (when run directly):
    python3 hooks/validate_skills.py              # validate every skill
    python3 hooks/validate_skills.py <name> ...   # validate only the named skills
    python3 hooks/validate_skills.py --report-only  # exit 0 even on failures

Wired as a pre-commit hook via .pre-commit-hooks.yaml in this repo.
Exits 0 on success, 1 on any failure with a per-violation report on stderr.
"""
