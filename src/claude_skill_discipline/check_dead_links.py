...
Usage (when run directly):
    python3 hooks/check_dead_links.py            # scan everything
    python3 hooks/check_dead_links.py path ...   # scan only the given files

Wired as a pre-commit hook via .pre-commit-hooks.yaml in this repo.
Exits 0 on clean, 1 with per-violation report on stderr.
"""
